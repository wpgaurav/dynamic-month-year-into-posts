...
export default function Edit( { attributes, setAttributes } ) {
	...
	return (
		<>
			<InspectorControls>
				<PanelBody title={ __( 'Countdown Settings', 'dynamic-month-year-into-posts' ) }>
					<SelectControl
						label={ __( 'Mode', 'dynamic-month-year-into-posts' ) }
						value={ mode }
						options={ [
							{ value: 'until', label: __( 'Days Until', 'dynamic-month-year-into-posts' ) },
							{ value: 'since', label: __( 'Days Since', 'dynamic-month-year-into-posts' ) },
						] }
					...
					<TextControl
						...
						type="date"
						help={ __( 'Select or enter the target date.', 'dynamic-month-year-into-posts' ) }
					/>

					<ToggleControl
						label={ __( 'Show Label', 'dynamic-month-year-into-posts' ) }
						checked={ showLabel }
						onChange={ ( newShowLabel ) => setAttributes( { showLabel: newShowLabel } ) }
					/>

					{ showLabel && (
						<TextControl
							label={ __( 'Label Text', 'dynamic-month-year-into-posts' ) }
							value={ label }
				...
				<PanelBody title={ __( 'Preview', 'dynamic-month-year-into-posts' ) } initialOpen={ false }>
					<p>
						<strong>{ __( 'Current countdown:', 'dynamic-month-year-into-posts' ) }</strong>
					</p>
					<p style={ { fontSize: '2em', fontWeight: 'bold' } }>{ displayText }</p>
					<p style={ { fontSize: '0.85em', color: '#757575' } }>
						{ __( 'This countdown will update live on the frontend.', 'dynamic-month-year-into-posts' ) }
					</p>
				</PanelBody>
			</InspectorControls>

			<span { ...blockProps }>
				{ targetDate ? displayText : __( 'Set a target date', 'dynamic-month-year-into-posts' ) }
			</span>
		</>
	);
}
